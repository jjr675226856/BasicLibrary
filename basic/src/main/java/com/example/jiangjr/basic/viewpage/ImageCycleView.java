package com.example.jiangjr.basic.viewpage;
 ...
 * @author minking
 */
//然后在页面中调用  {@link #setImageResources(ArrayList, com.example.jiangjr.times.widget.ImageCycleView.ImageCycleViewListener) }即可!
...
public class ImageCycleView extends LinearLayout {
	...
	public void setImageResources(ArrayList<String> imageUrlList, ImageCycleViewListener imageCycleViewListener) {
		...
		for (int i = 0; i < imageCount; i++) {
			...
			int imagePadding = (int) (mScale * 5 + 1.0f);
			mImageView.setPadding(imagePadding, imagePadding, imagePadding, imagePadding);
			mImageViews[i] = mImageView;
			mImageView.setLayoutParams(layoutParams);
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.mipmap.indicator_focused);
			} else {
				mImageViews[i].setBackgroundResource(R.mipmap.indicator);
			}
...
//			layoutParams.setMargins(10,0,0,0);//4个参数按顺序分别是左上右下
			...
			mImageIndex = index;
			// 设置图片滚动指示器背景
			mImageViews[index].setBackgroundResource(R.mipmap.indicator_focused);
			//mImageViews[index].setLayoutParams(layoutParams);
